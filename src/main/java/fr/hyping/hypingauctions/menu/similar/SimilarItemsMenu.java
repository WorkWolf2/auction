...
public class SimilarItemsMenu extends AbstractHAuctionMenu {

    public SimilarItemsMenu(HypingAuctions plugin, SimilarItemsMenuSession session) {
        super(plugin, "similar_items", session);
    }

    @Override
    public SimilarItemsMenuSession getSession() {
        return (SimilarItemsMenuSession) super.getSession();
    }
    ...
    public Map<String, String> getPlaceholderMap() {
        ...
        SimilarItemsMenuSession session = getSession();
        ...
        placeholders.put("{TOTAL_SIMILAR}", String.valueOf(session.getAllSimilarAuctions().size()));

        if (session.getTargetAuction() != null) {
            Component targetName = AutomaticMaterialTranslationManager.getInstance()
                    .getLocalizedComponent(session.getPlayer(), session.getTargetAuction().getItem());
            placeholders.put("{TARGET_ITEM}", LegacyComponentSerializer.legacySection().serialize(targetName));
        }
    ...
    private Map<String, String> createAuctionPlaceholders(Auction auction, Player viewer) {
        ...
        // Average price for similar items
