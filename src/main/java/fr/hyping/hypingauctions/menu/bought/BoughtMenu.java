...
public class BoughtMenu extends AbstractHAuctionMenu {
    ...
    // Cached once in postSlotsRead so postSlotsClean cannot destroy it.
    ...
    public void postSlotsRead(Player viewer) {
        ...
        if (cfg == null) {
            getPlugin().getLogger().warning("[BoughtMenu] Missing 'auction-item' template in config.");
            return;
        }
        ...
        if (raw == null) {
            getPlugin().getLogger().warning("[BoughtMenu] Template item is null at slot " + srcSlot
                    + ". Check that src-slot points to a valid slot in the menu YAML.");
            return;
        }
    ...
    public void postSlotsApply(Player viewer) {
        ...
        if (destSlots == null) {
            getPlugin().getLogger().warning("[BoughtMenu] Missing 'dest-slots.auction-items' in config.");
            setPlaceholders(getPlaceholderMap());
            return;
        }

        if (cachedTemplateItem == null) {
            getPlugin().getLogger().warning("[BoughtMenu] cachedTemplateItem is null — check src-slot in config.");
            setPlaceholders(getPlaceholderMap());
            return;
        }
    ...
    public Map<String, String> getPlaceholderMap() {
        ...
        BoughtMenuSession s = getSession();
        ph.put("{CURRENT_PAGE}",    String.valueOf(s.getPage()));
        ph.put("{MAX_PAGES}",       String.valueOf(s.getLastPage()));
        ph.put("{TOTAL_PURCHASES}", String.valueOf(s.getAuctionPlayer().getPurchases().size()));
        return ph;
    }
    ...
    private Map<String, String> createAuctionPlaceholders(Auction auction, Player viewer) {
        ...
        if (auction.getPurchaseDate() > 0) {
            ph.put("{PURCHASE_DATE}", Format.formatTime(auction.getPurchaseDate()));
        }
        return ph;
    }
}