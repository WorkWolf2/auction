...
public class HistoryMenu extends AbstractHAuctionMenu {

    public HistoryMenu(HypingAuctions plugin, HistoryMenuSession session) {
        super(plugin, "history", session);
    }

    @Override
    public HistoryMenuSession getSession() {
        return (HistoryMenuSession) super.getSession();
    }
    ...
    public Map<String, String> getPlaceholderMap() {
        ...
        HistoryMenuSession session = getSession();
        ...
        placeholders.put("{TOTAL_HISTORY}", String.valueOf(session.getAllHistory().size()));
    ...
    private Map<String, String> createAuctionPlaceholders(Auction auction, Player viewer) {
        ...
        if (auction.getPurchaseDate() > 0) {
            placeholders.put("{TRANSACTION_DATE}", Format.formatTime(auction.getPurchaseDate()));
        } else if (auction.getExpirationTime() > 0) {
            placeholders.put("{TRANSACTION_DATE}", Format.formatTime(auction.getExpirationTime()));
        }
